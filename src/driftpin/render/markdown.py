...
def _header_section(header: ArtifactHeader) -> str:
    lines = [
        ...
        f"- Source document hashes: {', '.join(header.source_doc_hashes) or '(none)'}",
        "",
    ]
    return "\n".join(lines)


def _traceability_section(result: PipelineResult) -> str:
    lines = ["## Traceability Matrix", "", "| Requirement ID | Title | Risk Tier | Coverage | Case IDs |",
              "|---|---|---|---|---|"]
    for row in result.traceability:
        case_ids = ", ".join(row.case_ids) if row.case_ids else "(none)"
        lines.append(
            f"| {row.requirement_id} | {row.requirement_title} | {row.risk_tier} | "
            f"{row.coverage_count} | {case_ids} |"
        )
    lines.append("")
    return "\n".join(lines)


def _scenarios_section(result: PipelineResult) -> str:
    lines = ["## Scenarios", ""]
    for scenario in result.strategy.scenarios:
        lines.append(f"### {scenario.scenario_id}: {scenario.title}")
        lines.append(f"- Requirement IDs: {', '.join(scenario.requirement_ids)}")
...
def _review_section(result: PipelineResult) -> str:
    lines = [
        "## Review",
        "",
        f"- Passed: {result.review.passed}",
        f"- Summary: {result.review.summary}",
        "",
    ]
    if result.review.findings:
        ...
        for finding in result.review.findings:
            requirement_ids = ", ".join(finding.requirement_ids) if finding.requirement_ids else "(none)"
            lines.append(
...
def render_markdown_report(result: PipelineResult, header: ArtifactHeader) -> str:
    sections = [
        ...
        _traceability_section(result),
        _scenarios_section(result),
        _test_cases_section(result),
