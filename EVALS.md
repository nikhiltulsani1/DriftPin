...
|---|---|---|
| Release 1 | **COMPLETE** (declared 2026-07-14) | Everything below was code-verified AND live-verified across NVIDIA/Groq (structural-only on local Ollama): enumerate-then-fill with code-enforced completeness (100% coverage on all 3 providers), the reviewer redesign (structural + split semantic + fallback + 504 handling; caught a real never-drop violation live), AC/NFR registry ingestion (deterministic parser + scoped LLM fallback + adjudication rule + 503-body classification), requirement-to-scenario completeness enforcement + the zero-coverage safety alarm (which caught a real scenario-ID collision bug live the day it shipped), and the registry-level spec consistency pass with the R2 coverage-gap round (silence-fix, lifecycle pairs, verdict hardening; adversarial answer-key score 5 full + 4 partial of 9, up from 0). Known limits carried into Release 2, documented rather than resolved: lifecycle-pair over-flagging on clean specs (precision item), the unlink-vs-GDPR model-capability miss, and human precision/recall scoring against the golden set — deliberately human-executed, never self-scored. |
| Release 2 | Not started | Planned: self-healing automation (Playwright/Selenium) with healing provenance, mutation scoring, requirement triage, `driftpin run`/`edit` CLI restructure, plus the lifecycle-pair precision fix carried over from Release 1. |
| Release 3 | Not started | Planned: OpenAI provider. |
| Release 4 | Not started | Planned: traceability-graph workspace UI. |
